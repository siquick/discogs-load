...
if ! RUST_BACKTRACE=1 ./target/release/discogs-load discogs_20240101_releases.xml.gz --db-host postgres.discogs-load.orb.local; then
...
if ! ./target/release/discogs-load --create-indexes --db-host postgres.discogs-load.orb.local; then
